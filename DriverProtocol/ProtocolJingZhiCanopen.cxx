...
/**
  ...
  * @retval None
  */
void CProtocolJingZhiCanopen::encode(uint8_t motor_idx, motorDriverCmdType ct, int32_t val, struct can_msg_s& msg)
{
...
/**
  ...
  */
bool CProtocolJingZhiCanopen::decode(motorDriverCmdType& ct, int32_t& val, const struct can_msg_s& msg)
{
	uint32_t tempVal = 0;
	
	if(memcmp(JINZHI_TARGET_SPEED_DATA, msg.cm_data, 8) == 0)
	{
	...
	for(int i = 0; i < 8; i++)
	{
		if((i < 4) && JINZHI_ACTURAL_POS_DATA[i] != msg.cm_data[i]) //compare bytes 0-3
	...
	for(int i = 0; i < 8; i++)
	{
		if((i < 4) && JINZHI_ERR_CODE_DATA[i] != msg.cm_data[i]) //compare bytes 0-3
			break;	//give up compare and jump to next for loop
		else if(i >= 4)	//copy bytes 4-7
		{
			tempVal >>= 8;
...
/**
  ...
  * @retval ret >= 0, Motor index
	*  				ret == -1, parse failed
  */
int32_t CProtocolJingZhiCanopen::getMotorIndex(const struct can_msg_s& Msg)
{
...
/**
  ...
  */
bool CProtocolJingZhiCanopen::getInitMsg(struct can_msg_s& msg)
{
	...
	if(msgCount > 0)
	{
		...
		msg.cm_hdr.ch_rtr = 1;
		msg.cm_hdr.ch_dlc = 8;
		memcpy(msg.cm_data, JINZHI_VEL_MODE_INIT_DATA, 8);
