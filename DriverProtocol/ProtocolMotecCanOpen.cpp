#include "ProtocolMotecCanOpen.h"
#include "Console.h"
#include "MessageTask.h"
namespace{
	const uint8_t INIT_MSG_COUNTS = 4;
	const uint8_t MOTEC_CAN_POS_MODE_TARGET_SPEED_DATA[8] = {0x60, 0xff, 0x60};
	const uint8_t MOTEC_CAN_ACTURAL_POS_DATA[8] = {0x43, 0x63, 0x60, 0x00, 0x00, 0x00, 0x00, 0x00};
	const uint8_t MOTEC_POS_MODE_INIT_DATA[INIT_MSG_COUNTS][8] = {{0x2B, 0x60, 0x60, 0x00, 0x03, 0x00, 0x00, 0x00},
																									{0x2B, 0x40, 0x60, 0x00, 0x86, 0x00, 0x00, 0x00},
																									{0x2B, 0x40, 0x60, 0x00, 0x07, 0x00, 0x00, 0x00},
																									{0x2B, 0x40, 0x60, 0x00, 0x0F, 0x00, 0x00, 0x00}};
	const uint8_t MOTEC_CAN_ERR_CODE_DATA[8] = {0x4B, 0x41, 0x60, 0x00};
}

CProtocolMotecCANOpen::CProtocolMotecCANOpen()
{
	isInitSend = false;
...
/**
  ...
  * @retval None
  */
void CProtocolMotecCANOpen::encode(uint8_t motor_idx, motorDriverCmdType ct, int32_t val, CanTxMsg& msg)
{
	...
	if (DRV_CMD_TARGET_SPEED == ct)
	{
		...
		int t = (val >= 0) ? 1 : -1;
		//unit:rpm
		int32_t speedVal = (int32_t)(val * 6.0f/(4.0f * encoderLines) + t * 0.5f);
		//Console::Instance()->printf("protocol motor send speed: 0x%x.\r\n", speedVal);
		msg.Data[0] = 0x2B;
		msg.Data[1] = 0xff;
		msg.Data[2] = 0x60;
		msg.Data[3] = 0x00;
...
/**
  ...
  */
bool CProtocolMotecCANOpen::decode(motorDriverCmdType& ct, int32_t& val, const CanRxMsg& msg)
{
	if (false == isInitSend)
	{
	...
	if(memcmp(MOTEC_CAN_POS_MODE_TARGET_SPEED_DATA, msg.Data, 8) == 0)
	{
			ct = 	DRV_CMD_TARGET_SPEED;
			return true; //compare finished
	}
	
	/* check for MOTEC_CAN_ACTURAL_POS */
	for(int i = 0; i < 8; i++)
	{
		if((i < 4) && MOTEC_CAN_ACTURAL_POS_DATA[i] != msg.Data[i]) //compare bytes 0-3
	...
	/* check for MOTEC_CAN_ERR_CODE */
	for(int i = 0; i < 8; i++)
	{
		if((i < 4) && MOTEC_CAN_ERR_CODE_DATA[i] != msg.Data[i]) //compare bytes 0-3
			break;	//give up compare and jump to next for loop
		else if(i >= 4)	//copy bytes 4-7
		{
			tempVal >>= 8;
...
/**
  ...
  * @retval ret >= 0, Motor index
	*  				ret == -1, parse failed
  */
int32_t CProtocolMotecCANOpen::getMotorIndex(const CanRxMsg& Msg)
{
...
/**
  ...
  */
bool CProtocolMotecCANOpen::getInitMsg(CanTxMsg& msg)
{
	...
	if(msgCount > 0)
	{
		...
		memcpy(msg.Data, MOTEC_POS_MODE_INIT_DATA[currentMsg%INIT_MSG_COUNTS], 8);
...
/**
  * @brief  parse error code and print
	...
	* @retval None
  */
void CProtocolMotecCANOpen::parseErrCode(uint8_t motorIdx, uint32_t errCode) const
{
...
/**
  ...
  * @retval motor Rx ID
  */
uint16_t CProtocolMotecCANOpen::calcRxID(uint16_t ID)
{
	return 0x600 + ID;
}
...
/**
  ...
  * @retval motor Tx ID
  */
uint16_t CProtocolMotecCANOpen::calcTxID(uint16_t ID)
{
	return 0x580 + ID;
}
